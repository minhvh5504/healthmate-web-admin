...
class DashboardService {
  ...
  async getDashboardStats(): Promise<BackendDashboardStats> {
    return apiClient<BackendDashboardStats>(
   ...
   * Get recent users
   */
  async getRecentUsers(): Promise<BackendRecentUser[]> {
    return apiClient<BackendRecentUser[]>(
      API_ENDPOINTS.STATISTICS.RECENT_USERS,
      {
        method: "GET",
      },
    );
